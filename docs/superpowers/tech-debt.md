...
## Дубли шрифтов в `public/{year}/fonts/` — рассмотрено, отклонено

- **Где:** `public/fonts/` + `public/{2021,2022,2022_2,2023,2024,2025}/fonts/`
- **Симптом:** 4 файла Gilroy + icons (woff/woff2) лежат в 7 локациях, байт-в-байт идентичны. Суммарный дубль ~1.6 MB.
- **Почему отложено (отклонено):** Архивные CSS (`public/{year}/css/style.min.css`) используют `url(../fonts/...)` относительно своей папки. Чтобы переехать на общую `/fonts/`, надо или править минифицированные legacy-CSS, или добавлять vercel rewrite, или symlink. Все варианты — нарушение изоляции архивов: сейчас каждый архив самодостаточный pixel-perfect снапшот, и общая папка превращается в источник конфликтов (правка/удаление файла ломает все архивы сразу). Экономия 1.6 MB не стоит риска.
- **Принятая конвенция:** новые сезоны (с 2026) — используют общую `public/fonts/` с абсолютными путями `/fonts/...` в CSS. Старые архивы — со своими копиями, не трогать. Case-sensitive проблема решается отдельным правилом в CLAUDE.md (имена Capitalized), а не объединением папок.
- **Как починить (если понадобится):** Переписать `url(../fonts/...)` → `url(/fonts/...)` в каждом архивном `style.min.css`, удалить `public/{year}/fonts/`. Проверить, что все архивы рендерятся одинаково с новой папкой. Делать только если иначе и так трогать архивы.
- **Найдено:** 2026-06-08

## Orphan files in public/img/intro/

- **Где:** `public/img/intro/intro.jpg`, `intro_2.jpg`, `2.webp`, `4.webp`, `650.png`, `650.webp`, `hand.png`, `hand.webp`, `logo.png`, `logo.webp`, `Coat_of_arms_of_Kalu.svg`
- **Симптом:** Файлы лежат в `public/img/intro/`, но не используются ни одним компонентом текущего сезона, ни одним из архивных HTML-снапшотов (`src/legacy/*/*.body.html`). Уцелели после миграции картинок в `src/assets/img/` (план A astro:assets). `intro/2026/` НЕ orphan — нужен `HistoryWidget` в архивах.
- **Почему отложено:** Не ломают сайт, не раздувают billable-трафик (Vercel кэширует, но никто не запрашивает). Уверенность что нигде не нужны — высокая, но не 100% (могут быть ссылки в RSS, шаринге, внешних сайтах).
- **Как починить:** Grep по проекту (`grep -rn "intro/intro\|intro/650\|intro/hand\|intro/logo\.\|intro/2\.webp\|intro/4\.webp"` в `src/`, `dist/`, `public/`) — убедиться что нигде нет. Удалить файлы. Проверить `npm run build` и обход страниц.
- **Найдено:** 2026-06-08
